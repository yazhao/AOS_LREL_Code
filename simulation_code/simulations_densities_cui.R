...
normal.null.rcv = matrix(data = NA, nrow = iter, ncol = 2*length(n)+1)
normal.null.rcv[,1] = 1:iter
...
for(i in 1:length(n)){
  ...
  for(k in 1:iter){
    ...
    Tstats = cui2018(X = x.mat, y = y.vec, beta = 0, delta = 0, Sigma = Sigma, small.sig = 1, T0 = TRUE)
    # store in matrix
    normal.null[k,i+1] = Tstats[2]
    normal.null[k,i+1+length(n)] = Tstats[3]
    normal.null.rcv[k,i+1] = Tstats[4]
    normal.null.rcv[k,i+1+length(n)] = Tstats[5]
  }
}

write.csv(normal.null, file = paste0(dir,"normal_null_cui_rho", cov_rho ,".csv"))
write.csv(normal.null.rcv, file = paste0(dir,"normal_null_cuircv_rho", cov_rho ,".csv"))
...
t.null[,1] = 1:iter
t.null.rcv = matrix(data = NA, nrow = iter, ncol = 2*length(n)+1)
t.null.rcv[,1] = 1:iter
...
for(i in 1:length(n)){
  ...
  for(k in 1:iter){
    ...
    t.null.rcv[k,i+1] = Tstats[4]
    t.null.rcv[k,i+1+length(n)] = Tstats[5]
  }
}

write.csv(t.null, file = paste0(dir,"t_null_cui_rho", cov_rho ,".csv"))
write.csv(t.null.rcv, file = paste0(dir,"t_null_cuircv_rho", cov_rho ,".csv"))
###############################################